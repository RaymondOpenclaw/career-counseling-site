...
interface SidebarLayoutProps {
  title: string;
  navItems: NavItem[];
  guardRole: 'admin' | 'counselor';
  children: React.ReactNode;
}

export default function SidebarLayout({ title, navItems, guardRole, children }: SidebarLayoutProps) {
  const { user, logout, loading } = useAuth();
  const { loading: guardLoading } = useRequireRole(guardRole, '/login');
      ...
      <div className="flex flex-1 flex-col">
        <header className="flex h-16 items-center gap-4 border-b bg-white px-4 lg:px-8">
          ...
          <div className="ml-auto text-sm text-muted-foreground">
            {guardRole === 'admin' ? '管理员' : '咨询师'}：{user?.username}
          </div>
        </header>
